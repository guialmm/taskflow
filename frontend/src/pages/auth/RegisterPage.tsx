      ...
      <div className="card w-full max-w-md p-8 animate-fade-in-up">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" className="input" placeholder="johndoe" {...register("username", { required: true })} />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-300">Password</label>
            <input type="password" className="input" placeholder="••••••••" {...register("password", { required: true, minLength: 6 })} />
          </div>
        ...
        <p className="mt-6 text-center text-sm text-slate-400">
          ...
          <Link to="/login" className="font-medium text-primary-400 hover:text-primary-300 hover:underline transition-colors">Sign in</Link>
        </p>
      </div>
    </div>
  );
